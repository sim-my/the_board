...
export const attendee = pgTable("Attendee", {
	id: integer().primaryKey().generatedAlwaysAsIdentity({ name: ""Attendee_id_seq"", startWith: 1000, increment: 1, minValue: 1, maxValue: 2147483647 }),
	userId: integer().notNull(),
	eventId: integer().notNull(),
	createdAt: timestamp({ precision: 3, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (table) => [
	uniqueIndex("Attendee_userId_eventId_key").using("btree", table.userId.asc().nullsLast().op("int4_ops"), table.eventId.asc().nullsLast().op("int4_ops")),
	foreignKey({
...
export const oneTimePassword = pgTable("OneTimePassword", {
	id: integer().primaryKey().generatedAlwaysAsIdentity({ name: ""OneTimePassword_id_seq"", startWith: 1000, increment: 1, minValue: 1, maxValue: 2147483647 }),
	email: text().notNull(),
	code: text().notNull(),
...
export const user = pgTable("User", {
	id: integer().primaryKey().generatedAlwaysAsIdentity({ name: ""User_id_seq"", startWith: 1000, increment: 1, minValue: 1, maxValue: 2147483647 }),
	email: text().notNull(),
	username: text(),
	name: text(),
	lastname: text(),
...
export const event = pgTable("Event", {
	id: integer().primaryKey().generatedAlwaysAsIdentity({ name: ""Event_id_seq"", startWith: 1000, increment: 1, minValue: 1, maxValue: 2147483647 }),
	posterUrl: text(),
